...
文件日志路径须对运行身份可写；容器持久化日志需挂载相应目录。`debug` 会额外记录请求/会话、文件写入、每个数据块的查重与加密、后端上传及 WebDAV 请求重试的开始和结束。关联字段 `op` 可把同一次请求的日志串起来；`chunk_id` 与 blob 名仅用于关联，不包含文件内容、口令、Cookie、认证头或完整后端 URL。调试日志量会随文件数和分块数增长，排障结束后应恢复为 `info`。
