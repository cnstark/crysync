...
| `max_upload_concurrency` | `0`（不限制） | 模块内 blob 并发上传后端的上限：`0` 不限制（缺省），`正数` 上限（跨连接/跨前端共享，限制 backend.Put 网络写；读与删除不受限） |
...
- **上传并发控制**：`max_upload_concurrency` 可限制模块内并发上传到后端的 blob 数（缺省不限制），跨连接/跨前端共享同一闸门，用于保护后端网络带宽/固定开销；读与删除不受限。
